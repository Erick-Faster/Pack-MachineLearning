...
previsores_test = onehotencoder2.fit_transform(previsores_test).toarray()

from keras.models import Sequential
from keras.layers import Dense

regressor = Sequential()
regressor.add(Dense(units = 13, activation = 'relu', input_dim = 13))
regressor.add(Dense(units = 8, activation = 'relu'))
regressor.add(Dense(units = 1, activation = 'linear'))

regressor.compile(loss = 'mean_absolute_error', optimizer = 'adam', metrics = ['mean_absolute_error'])
regressor.fit(previsores_train, classe_train, batch_size = 10, epochs = 500)

from sklearn.metrics import confusion_matrix, accuracy_score
previsoes_training = regressor.predict(previsores_train)
previsoes_test = regressor.predict(previsores_test)

precisao_training = accuracy_score(classe_train, previsoes_training)
matriz_training = confusion_matrix(classe_train, previsoes_training)


precisao_test = accuracy_score(classe_test, previsoes_test)
matriz_training = confusion_matrix(classe_test, previsoes_test)
